...
/home/hsbox/ztmp/
  ...
  run.sh                       本スクリプト（含める場合のみ）
```

### 3-1. run.sh は必須ではない（モジュールごとに必要性を検討）

`run.sh` は **モジュールごとに必要性を検討して使用する** スクリプトです。Plugin ファイルの単純な配置のみで動作する場合は、`run.sh` を省略できます。

### 3-2. run.sh が同梱されない場合の `make_hbx.py` の挙動

`tools/make_hbx.py` の **module モード**（サードパーティ Plugin のビルド）では、以下の **2 ファイルのみ** が `.hbx` にアーカイブされます。

```
<module_id>.hbx
  ├── <module_id>.py    ← 引数 plugin_file で指定したファイル
  └── version.txt       ← 2 行形式（module_id / version）
```

`run.sh` を `.hbx` に含めるかどうかは **make_hbx.py のコマンドライン引数では指定できません**。同梱したい場合は、ビルド後に zip ツール等で手動追加してください。

> 詳細は `docs/hbx_build_tool_guide.md` §3 を参照してください。

### 3-3. run.sh で行うべき処理

`run.sh` は以下のような **`.hbx` 展開先のデフォルト配置以外の処理** が必要な場合に使用します。

- Plugin ファイルを `plugins/` 直下ではなくサブディレクトリ等の特定の場所に配置したい
- 設定ファイル（`*_config.ini`）を初期化・マージしたい
- パーミッション設定（`chmod 640` 等）を行いたい
- インストール完了メッセージをユーザーに出したい
- サービス再起動を制御したい

`run.sh` を同梱しない場合、Plugin ファイル（`<id>.py`）は freeBox Loader 側のデフォルト処理で `plugins/` 直下に配置されます。それ以外の場所に配置したい・追加処理が必要な場合は **run.sh 内で明示的に処理を記述する必要があります**。
...
> Plugin ファイル構成は `plugin_dev_guide.md` §13 の規約に従い、設定ファイルは
> サブディレクトリ（`plugins/<id>/`）に配置することを推奨します。Uninstall 時の
> クリーンアップが確実に行われます。
