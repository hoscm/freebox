...
| 行 | フィールド | 意味 |
|----|---------|------|
| 1 | pdname | モジュール名（hsBox 側パッチ名） |
| 2 | obb | 適用可能な最低ベースビルド番号 |
| 3 | obv | 適用可能な最古の hsBox バージョン |
| 4 | nwv | 適用可能な最新の hsBox バージョン（リリース時に更新） |
...
> **任意フィールドの運用ルール整備予定**: index.json への任意フィールド追加
> （`requires_ffmpeg` / `hsbox_min_version` 等、Plugin 固有のメタ情報）の命名規則・
> 推奨フィールド集の標準化は **v1.1.0 で整備予定** です。v1.0.2 時点では Plugin
> 開発者が自由に追加できますが、Loader 側で解釈されることは保証されません
> （詳細は `docs/module_dev_guide.md` §6-3 を参照）。
...
- [ ] **ドキュメントの修正・更新内容がリリース対象に正しく反映されているか**（`docs/` 配下の各ガイド・脚注のバージョン表記等）
...
```bash
git add -A
git commit -m "chore(release): freeBox Loader vX.Y.Z (release notes summary)"
git push origin main
```

> **コミットメッセージ例**:
> - `chore(release): freeBox Loader v1.0.2 (docs revisions for module/plugin/loader-maintenance/run-sh guides)`
> - `chore(release): freeBox Loader v1.1.0 (default context resolution + reserved context registry)`

GitHub Web UI で Release を作成:

1. 「Releases」→「Draft a new release」
2. タグ: `v1.0.2`（リリースバージョンに合わせる）
...
> 上記は主要項目の例示です。完全な設定項目一覧（`[server]` セクションのポート/ホスト設定、
> `[loader].default_context`、`[status].notify_url` 等を含む）は配布物の
> `loader/conf/freebox_config.ini.template` を参照してください。
...
| `deploy: download_failed` | hbx ダウンロード失敗 | `index.json` の `repository` / `release_tag` / `id` を確認。詳細な原因分類と対処は `docs/hbx_build_tool_guide.md` §9 を参照 |
...
| エラー | 原因と対処 |
|--------|---------|
| `download_failed` | `index.json` の `release_tag` と GitHub Release のタグ名が一致しているか確認。詳細な原因分類は `docs/hbx_build_tool_guide.md` §9 を参照 |
