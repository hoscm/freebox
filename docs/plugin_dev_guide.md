...
- 予約済みコンテキスト（`/loader`, `/index`, `/api`, `/status`, `/manager` など）は使用できません
- 他の Plugin と同じプレフィックスを使用しないでください

> **予約済みコンテキストについて（v1.0.2 時点）**: freeBox Loader 内部で使用される
> コンテキストは今後の機能追加で拡張される可能性があります。Plugin ID 選定時は
> 予約済みコンテキスト一覧の最新版（`docs/specification.md` 参照）を確認してください。
...
> **絶対パスと Default Context の関係について（v1.0.2 時点）**: 上記例では
> `/freebox/myplugin/config` のように **freeBox 標準コンテキスト（`/freebox/`）を
> 前提とした絶対パス** を使用しています。
> hsBox 側で **Default Context を変更**（例: `/freebox/` → 別のコンテキストに切替）
> した場合、上記の絶対パスでは Plugin の API に到達できない可能性があります。
> 環境依存性を避けたい場合は、Manager UI の現在のパスから相対パスを構築するか、
> Plugin 起動時に有効なベース URL を取得する仕組みを検討してください。
> なお Default Context 切替に関連する到達性の問題については、**v1.1.0 で恒久対策を
> 検討中**です（現状は Default Context は変更しない運用が推奨されます）。
